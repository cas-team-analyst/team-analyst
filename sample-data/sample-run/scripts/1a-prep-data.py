...
def read_and_process_triangles():
    ...
    incurred = read_triangle_data(
        file_path=DATA_FILE_PATH + "input_data.xlsx",
        sheet_name="Inc 1",
        header_row=2,
        period_column=1,
        first_data_column=2,
    ...
    # No Reported Claims sheet in this data - skip
    reported = None
    ...
    closed = read_triangle_data(
        ...
        unit_type="Count",
        details="Workers Compensation - Closed Claims"
    )

    # Note: Exposure is NOT a triangle (no ages), so it's handled separately by other scripts.
    # Skip it from the main triangle processing.
    exposure = None

    # Concatenate all dataframes (filter out None entries)
    all_data = pd.concat([df for df in [incurred, paid, reported, closed, exposure] if df is not None], ignore_index=True)
