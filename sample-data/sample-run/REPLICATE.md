...
**Input files used:**
- `Triangle Examples 1.xlsx` — Workers Compensation triangle data; sheets: Paid 1 (Paid Loss), Inc 1 (Incurred Loss), Ct 1 (Reported Count), Exposure (Payroll 2001 actual + 2% growth formula)

**Scripts run:**
1. `scripts/1a-load-and-validate.py` — Read raw data and create canonical triangle format
   - **Customizations made:** Implemented read_triangle_data() to handle two distinct header layouts (2-row header for Paid/Incurred, 1-row header for Count). Exposure values computed via 2% annual growth formula from 2001 base ($316,377,370). Count triangle classified as Reported Count (no Closed qualifier in source).
...
**Data validation:** User confirmed data format on 04/27/2026
