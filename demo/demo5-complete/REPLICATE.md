...
**Input files used:**
- `raw-data/canonical-triangles.xlsx` (18 KB, last modified 2026-04-01) — 5 sheets: Incurred, Paid, Reported, Closed, Exposure; 10 AYs (2015–2024) x 10 dev ages (12–120 months)
- `raw-data/canonical-elrs.xlsx` (9.4 KB, last modified 2026-04-01) — ELR sheet with Expected Loss Rate and Expected Frequency for 10 accident periods

**Scripts run:**
1. `scripts/1a-prep-data.py` — Read raw data and create canonical triangle format
   - **Customizations made:** (1) Set `EXPECTED_LOSS_RATES_FILE = "canonical-elrs.xlsx"`; (2) Added column rename in `read_and_process_expected_loss_rates()`: 'Accident Period'→'period', 'Expected Loss Rate'→'expected_loss_rate', 'Expected Frequency'→'expected_freq'
   - **Output:** `processed-data/1_triangles.parquet`, `processed-data/1_expected_loss_rates.parquet`
...
4. `scripts/1d-averages-qa.py` — Calculate LDF averages (simple, weighted, exclude high/low × all/3yr/5yr/10yr windows) and CV/slope diagnostics
   - **Output:** `processed-data/4_ldf_averages.parquet`

**Data validation:** User confirmed data format on 2026-04-21
